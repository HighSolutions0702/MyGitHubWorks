...
class Campaign < ActiveRecord::Base
  ...
  def get_avg
    total_hit = 0
    date_wise_hit = self.hits.count(:all, group: "DATE(created_at)")
    if date_wise_hit.present?
      date_wise_hit.each do |key, value|
        total_hit = total_hit + value
      end
    end
    if total_hit > 0 && self.hits.present?
      date_range = (Date.today.to_date - self.hits.first.created_at.to_date).to_i
      return (total_hit.to_f / date_range.to_f).to_f
    else
      return 0
    end
  end
