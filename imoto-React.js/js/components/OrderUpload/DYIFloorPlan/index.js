...
class DYIFloorPlan extends Component {
  constructor(props) {
    super(props)
  ...
  onRemovePhoto() {
    ...
    new Promise((resolve, reject) => {
      dispatch({ type:types.REMOVE_SELECTED_DYI_FLOOR_PLAN, payload:{ id:floorPlans[0].id, orderProductId:product.id, resolve, reject } })
    }).then(() => {
      this.setState({
        update:true
      })
  ...
  onDrop(files) {
    new Promise((resolve, reject) => {
      this.props.dispatch({
        type: types.TRY_SELECT_DYI_FLOOR_PLAN,
        payload: {
...
function select(state) {
  return {
    floorPlans: state.orderUpload && state.orderUpload.selectedDYIFloorPlan
  }
}
export default connect(select)(cssModules(DYIFloorPlan, styles))
